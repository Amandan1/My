...
function Tree_treeView() {
	...
	this.target="_blank";
	this.click_expand=true;
	this.treeimg_click=false;
	//----------------
	this.selectID=null;
	...
	this.expand=function expand(nodeID,isExpand){
		...
		var td=document.getElementById("Tree_ltd_"+nodeID);
		if (td==null) td=document.getElementById("Tree_td_"+nodeID);
		if(node){
...
function Tree_node(text,isleaf,hint,status,img1,img2,action) {
	...
	this.refresh=function refresh(){
		...
		var html="<DIV id='Tree_treeView_"+this.action.treeViewIndex+"'>";
	...
	this.setObj_prop=function setObj_prop(obj_prop) 	 {
		this.obj_prop=obj_prop;
	 }
}
//fire when click node
function Tree_on_action(action) {
...
function Tree_expandNode(nodeID) {
    var node=Tree_node_array[nodeID];
...
//init treeView from array
function Tree_buildTree(node,treeView) {
...
function Tree_table(node) {
	...
	if(tree.showLine){
		...
		var temmode=tree.click_expand?"Tree_clickNode":"Tree_expandNode";
		html+="<TD oncontextmenu='return Tree_treeView_array["+tree.id+"].callback_rightClick("+node.id+
				...
				")' onclick="+temmode+"("+node.id+") style='cursor: pointer;cursor: hand;' id='Tree_ltd_"+node.id+"'>"+
...
function Tree_td(node) {
	var tree=node.getTreeView();
	var temmode=tree.click_expand?"Tree_clickNode":"Tree_exeNode";
	var ret = "<TD oncontextmenu='return Tree_treeView_array["+tree.id+"].callback_rightClick("+node.id+
				")' ondblclick='return Tree_treeView_array["+tree.id+"].callback_dblClick("+node.id+
				")' onclick="+temmode+"("+node.id+") style='cursor: pointer;cursor: hand;'  NOWRAP id='Tree_td_"+node.id+"' onmouseover=Tree_mouseover(this,"+node.id+
