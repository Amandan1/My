                                 ...
                                 {if (!chnode.parent.checkbox.checked) chnode.parent.checkbox.checked=true;}
                                  else
                                   {
                                    ...
                                    for (var i=0;i<chnode.parent.childCount;i++)
                                     ...
                                     {if ((chnode.parent.child[i])&&(chnode.parent.child[i].checkbox)&&(chnode.parent.child[i].checkbox.checked)) {pchecked=true;continue;}}
...
function Tree_node(text,hint,status,img1,img2,action)
{
	...
	this.setData=function setData(data)
	 ...
	 this.addCheckBox=function addCheckBox(name,type,defaultcheck,disabled)
	 {
	   ...
	   //var fhtml="<input type="+type+" hidefocus=true class=nochk name="+name+" value="+this.type+"-"+this.data+" id=tree_checked_"+this.id+" onmousedown=' return Tree_treeView_array["+temid+"].callback_chClick("+this.id+");' onpropertychange='return Tree_treeView_array["+temid+"].callback_checked("+this.id+")' ";
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		else
		{
			...
			if(action.link!="") try{eval(command);}catch(e){alert("??¡À??????¨ª?¨®\r\n\r\n"+e.description);}
		}
	}
	else if(action.type==Tree_SCRIPT)//run script
	{
		try{eval(action.script);}catch(e){alert("??¡À??????¨ª?¨®\r\n\r\n"+e.description);}
	}
}
//----------------
