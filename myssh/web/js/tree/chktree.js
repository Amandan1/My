//write by hrmis.cyq 2004.11.12
//modify by YQM 2005-07-29
var Tree_CHILD=-1;
var Tree_LAST=-2;
var Tree_ROOT=-3;
var Tree_FIRST=-4;
...
{	
	...
	this.folderClass1=this.folderClass2=this.folderClass3=
		this.fileClass1=this.fileClass2=this.fileClass3=this.filecheckedclass=this.foldercheckedclass="";
	this.target="_blank";
	this.mutilchecked=false;
	//----------------
	this.selectID=null;
	...
	this.container=new Tree_node("",false,"","","","",action);
	...
	this.callback_dblClick=function callback_dblClick(nodeID){return true;}
	this.callback_chClick=function callback_chClick(nodeID){var tnode=tree.getNode(nodeID);this.checklevel=tnode.treelevel;return true;}
	this.callback_checked=function callback_checked(nodeID) {
		var chnode=this.getNode(nodeID);
		if (!(chnode.checkbox)) return true;
		var ftd=document.getElementById("Tree_td_"+nodeID);
		if (chnode.checkbox.checked) {
			if (chnode.childCount>0) ftd.className=this.foldercheckedclass;
			else ftd.className=this.filecheckedclass;
		}
		else {
			if (chnode.childCount>0) ftd.className=this.folderClass1;
			else ftd.className=this.fileClass1;
		}  
		if (this.mutilchecked) {
			if (chnode.treelevel>=this.checklevel) {  	             
				if (chnode.childCount>0) {
					for (var i=0;i<chnode.childCount;i++) {
						if (chnode.child[i].checkbox) if (chnode.child[i].checkbox.checked!=chnode.checkbox.checked) {
							chnode.child[i].checkbox.checked=chnode.checkbox.checked;
						}
					}
				}
			}
			if (chnode.treelevel<=this.checklevel) {
				if ((chnode.treelevel>1)&&(chnode.parent.checkbox)) {
					if (chnode.checkbox.checked) {
						if (!chnode.parent.checkbox.checked) chnode.parent.checkbox.checked=true;
					}
					else {
						var pchecked=false;
						for (var i=0;i<chnode.parent.childCount;i++) {
							if ((chnode.parent.child[i])&&(chnode.parent.child[i].checkbox)&&(chnode.parent.child[i].checkbox.checked)) {
								pchecked=true;continue;
							}
						}
						if (chnode.parent.checkbox.checked!=pchecked) chnode.parent.checkbox.checked=pchecked;
					}
				}
			}
		}
		return this.node_onchecked(nodeID);
	}
	this.node_onchecked=function node_onchecked(nodeID){return true;}
	//---------	
	...
	this.getSelect=function getSelect(){
	...
	this.add=function add(relate_ID,nOption,nIndex,text,isleaf,hint,status,img1,img2){
		if(nOption==Tree_ROOT)	{
			return this.container.addChild(nIndex,text,isleaf,hint,status,img1,img2);
		}
		if(nOption==Tree_CHILD){
			var parent=this.getNode(relate_ID);
			if(parent==null) return null;
			return parent.addChild(nIndex,text,isleaf,hint,status,img1,img2);
		}
		if(nOption==Tree_SIBLING){
			if(relate_ID==this.container.id) return null;
			var node=this.getNode(relate_ID);
			if(node==null) return null;
			return node.addSibling(nIndex,text,isleaf,hint,status,img1,img2);
		}
		return null;
	}
...
//-----------------------------
function Tree_node(text,isleaf,hint,status,img1,img2,action)
{
	...
	this.selected=false;
	//是否获得了子节点(无子节点的算已经获得)
	this.getchild=typeof(isleaf)=="boolean"?isleaf:true;
	this.index=0;
	this.childCount=0;
	this.type=0;
	this.data=0;
	...
	//------------------
	this.addChild=function addChild(index,text,isleaf,hint,status,img1,img2){
		var action=new Tree_action(0,0,0,0,this.action.treeViewIndex);
		var node=new Tree_node(text,isleaf,hint,status,img1,img2,action);
		this.add(node,index);
		return node;
	}
	...
	this.addSibling=function addSibling(index,text,isleaf,hint,status,img1,img2){
		if(this.parent==null)return null;
		if(index==Tree_PREV) index=this.index;
		if(index==Tree_NEXT) index=this.index+1;
		return this.parent.addChild(index,text,isleaf,hint,status,img1,img2);
	}
	this.add=function add(childNode,index){
		...
		childNode.treelevel=this.treelevel+1;
		this.childCount++;
	...
	//-------------
	this.getParentByType=function getParentByType(parentType)
	{
	  if (parentType==null) return null;
	  var tmp=this.parent;
	  while (tmp!=null)
	  {
	     if (tmp.type==parentType)
	     { 
	     break;
	     }
	     else
	...
	//--------------
	this.getNodeFullPathText=function getNodeFullPathText()
	{
	  var tmptext=this.text;
	  var tmp=this.parent;
	  while (tmp!=null)
	   {
	    tmptext=tmp.text+"/"+tmptext;
	    tmp=tmp.parent;
	   }
	  return tmptext;
	}
	...
	this.setData=function setData(data)
	 ...
	 this.addCheckBox=function addCheckBox(name,type,defaultcheck,disabled)
	 {
       if (this.checkbox) return;
	   var temid=this.getTreeView().id;
	   var fhtml="<input type="+type+" hidefocus=true class=nochk name=\""+name+"\" value=\""+this.data+"\" id=tree_checked_"+this.id+" onmousedown=' return Tree_treeView_array["+temid+"].callback_chClick("+this.id+");' onpropertychange='return Tree_treeView_array["+temid+"].callback_checked("+this.id+")' ";
	   if (defaultcheck)
	   {
	     fhtml+=" checked";
	   }
       ...
       fhtml+=" />";
	   var _td=document.getElementById("Tree_td_check_"+this.id);
	   _td.innerHTML=fhtml;
	   this.checkbox=document.getElementById("tree_checked_"+this.id);
	   return this.checkbox;
	 }

	 this.removeCheckBox=function removeCheckBox()
	 {
     var _td=document.getElementById("Tree_td_check_"+this.id);
	 _td.innerHTML="";
	 this.checkbox=null;
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		else
		{
			...
			if(action.link!="") try{eval(command);}catch(e){alert("??±??????í?ó\r\n\r\n"+e.description);}
		}
	}
	else if(action.type==Tree_SCRIPT)//run script
	{
		try{eval(action.script);}catch(e){alert("??±??????í?ó\r\n\r\n"+e.description);}
	}
}
//----------------
...
function Tree_clickNode(nodeID)
{
	...
	var td=document.getElementById("Tree_td_"+nodeID);
	//---------
	if(!node.expanded && (node.childCount>0 || node.getchild==false) &&tree.flag==false)
...
function Tree_buildTree(node,treeView)//init treeView from array
{
	...
	while( index <= nLen )
	{
		...
		case Tree_const_file:			
			...
			nodeChild=new Tree_node(node[index+1],false,node[index+2],node[index+3],node[index+4],node[index+4],action);
		...
		case Tree_const_folder:	
			...
			nodeChild=new Tree_node(node[index+1],false,node[index+2],node[index+3],node[index+4],node[index+5],action);
			if(count==0) treeView.container.add(nodeChild,Tree_LAST);
			else nodeParent.add(nodeChild,Tree_LAST);
...
function Tree_table(node)
{
	...
	if(tree.showLine){
		...
		html+="<TD oncontextmenu='return Tree_treeView_array["+tree.id+"].callback_rightClick("+node.id+
				")' ondblclick='return Tree_treeView_array["+tree.id+"].callback_dblClick("+node.id+
	...
	html+="<TD id=Tree_td_check_"+node.id+"></TD><TD oncontextmenu='return Tree_treeView_array["+tree.id+"].callback_rightClick("+node.id+
				")' ondblclick='return Tree_treeView_array["+tree.id+"].callback_dblClick("+node.id+
...
function Tree_mouseout(obj,nodeID)
{
	...
	else if ((node.checkbox)&&(node.checkbox.checked))
	    obj.className=node.childCount>0?tree.foldercheckedclass:tree.filecheckedclass;
...
function Tree_GetLineImg(node) {
	...
	if(!node.getchild || node.childCount>0){
		if(node.expanded)
