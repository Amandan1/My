 ...
 * 修改历史：
 * 修改日期：2005-12-20
 * 修改内容：由objtree.js更名为tree.js，增加注释。
 * 修改作者：YQM
 * @author YQM
 * @version 1.0
 */
...
function Tree_node(text,hint,status,img1,img2,action)
{
	...
	//-------------
	this.getParentByClsid=function getParentByClsid(parentClsid)
	{
	  if (parentClsid==null) return null;
	  var tmp=this.parent;
	  while (tmp!=null)
	  {
	     if (tmp.type==parentClsid)
	     { 
	     break;
	     }
	     else
