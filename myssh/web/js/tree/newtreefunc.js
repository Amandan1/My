...
{try{
	...
	this.err=""
	iml=this
	this.copywrite=" &nbsp;2004，All right reserved!"
...
{try{
	...
	this.copywrite="Copywrite &nbsp;2003，All right reserved!"
	this.err=""
	...
	{	if(srcNode.hasChild && showExpand)
		{	var sEx=srcNode.next==null?"_end":"";
			...
			//add by macro---------------
			srcNode.exIcon.align="absMiddle";
...
//return addNode(node,"last",text,key,img,exeArg,_url)
	var addNode=function(toNode,relation,text,key,ico,exeCategory,exeArg)
	{
		...
		var label=document.createElement("<SPAN  class=\"TreeNode\" >");
		container.style.cssText="margin:0px;padding:0px;white-space:nowrap;"
		with(nbody)
		{	insertAdjacentElement("beforeEnd",label)
			style.cssText="cursor: pointer;cursor: hand;text-align:left;color:black;font-size:9pt;margin:0;"
		...
		if(typeof(ico)=="string")
			if(ico.toLowerCase().indexOf(".gif")!=-1 || ico.toLowerCase().indexOf(".jpg")!=-1){ico=imageList.add(ico).index;}
		if(icons[ico]!=null)
		{	icon.src=icons[ico].src;
			
		  icon.align ="absmiddle";//add by macro---------------
		  
			nbody.insertAdjacentElement("afterBegin",icon)
		}
		label.innerHTML=text
		//label.style.cssText=labelStyle!=null?labelStyle:"padding:0;margin-left:2;vertical-align:middle;text-align:left;"
		with(label)
		{
			...
			onmouseover=function(){if(event.fromElement!=node.icon)if(run(node.onmouseover))run(tree.onmouseover,node);
				//add by macro---------------
				if(this.style.background!="highlight")this.style.background="yellow";
				window.status=text;
			}
			onmouseout=function(){if(event.toElement!=node.icon)if(run(node.onmouseout))run(tree.onmouseout,node);
				//add by macro---------------
				if(this.style.background!="highlight")this.style.background="";
				window.status="";
			}
			...
			//oncontextmenu=function(){run(node.select);if (run(node.oncontextmenu))run(tree.oncontextmenu,node);}
			oncontextmenu=function(){run(node.select);window.event.cancelBubble=true;if (run(node.oncontextmenu))run(tree.oncontextmenu,node);return false;}
		}
		with(icon)
		{
			onclick=function(){run(node.select);if(run(node.onclick))if(run(tree.onclick,node))run(node.execute);}
		...
		node.oncontextmenu=new Function("return true;");
		...
		if(showBranch){
			...
			nbody.insertAdjacentElement("afterBegin",node.exIcon);
		}
		node.lineIcon=[]
		if(showLine)
		{	for(var i=tier-2;i>=0;i--){
			...
			img.align="absMiddle";
			//end by macro---------------
			nbody.insertAdjacentElement("afterBegin",img);
			node.lineIcon[i]=img;
		}
		...
		if(ma!=root){
			if(ma.exIcon==null && showExpand){
				...
				//add by macro---------------
				ma.icon.align="absMiddle";
				//end by macro---------------
				o.insertAdjacentElement("beforeBegin",ma.exIcon);
			}
			
			setExIcon(ma);
			if(ma.exIcon){ma.exIcon.onmousedown=function(){ma.expand();}}
		}
		...
		//example	
		//toNode1.addDLink('gdsc/jg/jhgl/sngdgh/sngdjh.html','三年滚动规划','_blank','file','n105010100');

        node.addDLink=function(_url,text,target,img,key){
        	if(text==null||text=="")text=_url;
        	var exeArg="url";
        	if(target!=null&&target!="")exeArg+=":"+target;
        	if((img==null||img==""))img="link";
        	return addNode(node,"last",text,key,img,exeArg,_url)
        	}
	//example	
	//var addNode=function(toNode,relation,text,key,ico,exeCategory,exeArg)
		...
		{	case "expand":
			...
			default:
				...
				{	if(typeof(exeArg)!="string"){break;}
					if(exeCategory.indexOf("url:")!=-1)
					{	var tar=exeCategory.replace("url:","");
					    node.execute=function(){
						                        open(exeArg,tar, "toolbar=yes,location=yes,directories=yes,status=yes,menubar=yes,scrollbars=auto,resizable=yes,height=" + window.screen.availHeight + ",width=" + window.screen.availWidth +",top=0,left=0");
						                       }
					}else {	
					  node.execute=function(){
					    open(exeArg,tree.target);
	...
	root.addDLink=function(_url,text,target,img,key){if(text==null||text=="")text=_url;var exeArg="url";if(target!=null&&target!="")exeArg+=":"+target;if((img==null||img==""))img="link";return addNode(root,"last",text,key,img,exeArg,_url)}
