	...
	$(function() {
		var loginFun = function() {
			if ($('#loginDialog form').form('validate')) {
				...
				$.post(myhope.contextPath + '/base/user!nssnsc_login.myhope', $('#loginDialog form').serialize(), function(result) {
					if (result.success) {
			...
			buttons : [ {
				...
				handler : function() {
					if ($('#passwordDialog form').form('validate')) {
						$.post(myhope.contextPath + '/base/user!nsc_updateCurrentPwd.myhope', {
							'data.pwd' : $('#pwd').val()
						}, function(result) {
							if (result.success) {
		...
		mainMenu = $('#mainMenu').tree({
			url : myhope.contextPath + '/base/resource!nsc_getMainMenu.myhope',
			parentField : 'pid',
			lines : true,
			animate : true,
		...
		mainTabsMenu = $('#mainTabsMenu').menu({
			onClick : function(item) {
				...
				if(type === 'closeOther' || type === 'closeAll'){
		...
		var _mainTabs_flag_isAdd = false;
		mainTabs = $('#mainTabs').tabs({
			fit : true,
			border : false,
			onSelect : function(title,index) {
				if(!_mainTabs_flag_isAdd){
					var tab = mainTabs.tabs('getTab', index),
						opt = tab.panel('options'),
						node = mainMenu.tree('find', opt.nodeId);
					if(node)
						mainMenu.tree('select', node.target);
				}
				_mainTabs_flag_isAdd = false;
			},
			onAdd : function(title,index) {
				_mainTabs_flag_isAdd = true;
			},
