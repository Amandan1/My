			...
			columns : [ [ {
				...
				formatter : function(value, row) {
					...
					<%if (securityUtil.havePermission("/base/user!grantRole")) {%>
						str += myhope.formatString('<img class="iconImg ext-icon-user" title="员工角色" onclick="grantRoleFun(\'{0}\');"/>', row.id);
					<%}%>
					<%if (securityUtil.havePermission("/base/user!grantOrganization")) {%>
						str += myhope.formatString('<img class="iconImg ext-icon-group" title="员工机构" onclick="grantOrganizationFun(\'{0}\');"/>', row.id);
					<%}%>						
...
<body class="easyui-layout" data-options="fit:true,border:false">
					...
					<form id="searchForm">
						<table>
							<tr>
								...
								<td>性别</td>
								<td>
								<input name="QUERY_t#sex_S_EQ" class="easyui-combobox" data-options="valueField:'key',textField:'val',url:'${pageContext.request.contextPath}/base/metadata!nssnsc_findByName.myhope?data.name=sex',width:80,panelHeight:'auto',editable:false" />
								<!-- <select name="QUERY_t#sex_S_EQ" class="easyui-combobox" data-options="panelHeight:'auto',editable:false"><option value="">请选择</option>
	...
	<div id="menu" class="easyui-menu" style="width:120px;display: none;">
		...
		<%}%>
		<%if (securityUtil.havePermission("/base/user!grantRole")) {%>
		<div onclick="grantRoleFun();" data-options="iconCls:'iconImg ext-icon-user'">员工角色</div>
		<%}%>
		<%if (securityUtil.havePermission("/base/user!grantOrganization")) {%>
		<div onclick="grantOrganizationFun();" data-options="iconCls:'iconImg ext-icon-group'">员工机构</div>
		<%}%>
		<%if (securityUtil.havePermission("/base/user!delete")) {%>
