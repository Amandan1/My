...
public class UserServiceImpl extends BaseServiceImpl<TUser> implements UserServiceI {
	...
	@Override
	public Long countByFilter(HqlFilter hqlFilter, String searchOrg) {
		String hql = "select count(distinct t) from TUser t";
		if(searchOrg != null) {
			hql = "select count(distinct t) from TUser t inner join t.organizations o";
		}
		return count(hql + hqlFilter.getWhereHql(), hqlFilter.getParams());
	}

	@Override
	public List<TUser> findByFilter(HqlFilter hqlFilter, String searchOrg, int page, int rows) {
		String hql = "select distinct t from TUser t";
		if(searchOrg != null) {
			hql = "select distinct t from TUser t inner join t.organizations o";
		}
