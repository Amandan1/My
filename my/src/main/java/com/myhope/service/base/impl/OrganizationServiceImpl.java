...
public class OrganizationServiceImpl extends BaseServiceImpl<TOrganization> implements OrganizationServiceI {
    ...
    public List<TOrganization> findOrgByPID(List<TOrganization> treeList, String pid, List<TOrganization> childOrg) {
        // 根据当前机构查询子孙孙孙孙孙机构
        for (TOrganization org : treeList) {
            //遍历出父id等于参数的id，add进子节点集合
            if(org.getOrganization() != null) {
                if (org.getOrganization().getId().equals(pid)) {
                    //递归遍历下一级
                    findOrgByPID(treeList, org.getId(), childOrg);
                    childOrg.add(org);
                }
            }
        }
    ...
    public List<TOrganization> findOrg() {
        List<TOrganization> treeList = find("from TOrganization");
        return treeList;
    }
