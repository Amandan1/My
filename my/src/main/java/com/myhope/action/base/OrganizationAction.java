...
public class OrganizationAction extends BaseAction<TOrganization> {
    private static final long serialVersionUID = 9036129424220809777L;
    ...
    public void save() {
        Json json = new Json();
        if (data != null) {
            if (data.getReferee() != null) {
                Map<String, Object> params = new HashMap<String, Object>();
                params.put("referee", data.getReferee());
                TUser user = userService.getByHql("from TUser u where u.code = :referee", params);
                if (user == null) {
                    json.setMsg("推荐人编号填写错误，请重新填写！");
                    writeJson(json);
                    return;
                }
            }
            if (data.getOrgmanager() != null) {
                Map<String, Object> params = new HashMap<String, Object>();
                params.put("orgmanager", data.getOrgmanager());
                TUser user = userService.getByHql("from TUser u where u.code = :orgmanager", params);
                if (user == null) {
                    json.setMsg("管理员编号填写错误，请重新填写！");
                    writeJson(json);
                    return;
                }
            }
     ...
     * 获得当前用户能看到的所有机构树
     */
    public void nsc_getOrganizationsTree() {
        SessionInfo sessionInfo = (SessionInfo) getSession().getAttribute(ConfigUtil.getSessionInfoName());
     ...
     * 获得当前用户的机构
     */
    public void nsc_getOrganizationByUserId() {
     ...
     * 获得当前班次的机构
     */
    public void nsc_getOrganizationByScheduleId() {
        HqlFilter hqlFilter = new HqlFilter(getRequest());
