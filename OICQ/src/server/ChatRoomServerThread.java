...
/**
 * @author Cuiys
 * @ClassName:ChatRoomServerThread
 * @Version 版本
 * @ModifiedBy 修改人
 * @Copyright 公司名称
 * @date 2018年1月18日 上午10:52:38
 */
public class ChatRoomServerThread implements Runnable {
	...
	public ChatRoomServerThread(ServerClientConnectionStream userCS,
			Set<UserInformation> userSet,
			Map<String, ServerClientConnectionStream> userMap,
			JTextArea hintInfo, JTextArea startInfoTextArea,
		...
		while (true) {
			...
			if (  message.indexOf("%LOGIN%")==0) {
				...
				if (userMap.containsKey(userInfo.getAccount())) {
					...
					userCS.send("LOGIN_SUCCESSFULLY");
					startInfoTextArea.append("客户端账号" + userInfo.getAccount()
							+ "登录服务器登陆成功!\n");
					userList.append(userInfo.getAccount() + "&LOGIN");
					sendCurrentUserList();
				}
			} else if (message.indexOf("%EXIT%") == 0) {
				startInfoTextArea.append("已向客户端账号" + userInfo.getAccount()
						+ "发送了用户列表消息\n");
				break;
			}
		}
	...
	public void sendCurrentUserList() {
		...
		for (UserInformation userInfo : userSet) {
			onlineUserInfo.append("账号" + userInfo.getAccount() + ",密码"
					+ userInfo.getPassword() + ",昵称:" + userInfo.getName()
					+ ",IP地址" + userInfo.getIP() + ",端口号(UDP)"
					+ userInfo.getPort() + "\n ");
			userList.append(userInfo.getIP() + "-" + userInfo.getPort() + "-"
					+ userInfo.getAccount() + "-" + userInfo.getName() + "-"
					+ userInfo.getUserPortraitNum() + ".");
		}
		for (String string : userMap.keySet()) {
			userMap.get(string).send("%USER_LIST%:" + userList);
			startInfoTextArea.append("服务器向" + string + "发送的用户列表为:%USER_LIST%"
					+ userList + "\n");
